...
def get_trending_albums():
    ...
    # Execute a query to fetch trending albums with their IDs
    cursor.execute("SELECT AlbumID, Title FROM Album LIMIT 4")
    trending_albums = [{'id': album[0], 'title': album[1]} for album in cursor.fetchall()]
    ...
    if request.method == 'GET':
        ...
        if query:
            ...
            if mydb:
                ...
                album_query = "SELECT AlbumID, Title, 'Album' as category FROM Album WHERE title LIKE %s"
                param = ('%' + query + '%',)
                ...
                album_results = [{'id': album[0], 'title': album[1], 'category': album[2]} for album in cursor.fetchall()]
